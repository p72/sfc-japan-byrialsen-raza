...
| 5 | 財政・金利ショックを解き、ベースラインからの乖離を見る（`shocks.py`） | できた |
...
| `final_test.py` | 1997〜2023年度を通しで動学解し、誤差率を出す。`dk_final.png` を書く |
| `shocks.py` | 政府消費・公共投資・金利のショックを解き、ベースラインからの乖離を出す。`dk_shocks.md` `dk_shocks.png` を書く |
...
- 実質の需要項目は誤差率 1〜7% で追える。推定式14本の小さなモデルとしては
  十分で、需要主導の SFC モデルが日本の実物側を説明できることを示している
- **労働市場は安定している**。失業率の誤差率 10.1%、就業者数 0.4%。就業者を
  実質GDPと労働力人口で直接説明し、賃金を失業率の変化のフィリップス曲線で
  決めるだけなので、生産関数から GDP ギャップを作る型のモデルのように失業率が
  暴れることがない
- **物価と名目値は粗い**。消費者物価と輸出価格の2本しか価格の式が無く、
  GDPデフレータは名目÷実質の結果として出るだけなので、誤差率が 3〜4% になる。
  名目GDP・賃金・家計純資産の誤差はここから来ている
...
入れて、ベースライン（2010年度起点の動学解）からの乖離を見た。表の全体は
`dk_shocks.md`、図は `dk_shocks.png`。

| ショック | 指標 | 乖離 |
|---|---|---:|
| 実質政府消費 +5兆円 | 実質GDP乗数（1年目 → 6年目） | 1.00 → 1.02 |
| | 政府金融純資産（11年目、兆円） | −40 |
| 実質公共投資 +5兆円 | 実質GDP乗数（1年目 → 6年目） | 0.66 → 0.52 |
| 金利 +1%ポイント | 実質GDP（11年目、%） | +0.23 |
| | 実質消費 ／ 設備投資（11年目、%） | +0.63 ／ +0.19 |
| | 政府金融純資産（11年目、兆円） | −52 |

読み方。

- **政府消費の乗数は 1.0 前後で減衰しない**。WP の輸入関数は民間需要（c + i + x）
  だけを説明変数にしているので、政府消費は輸入に漏れない。加えて設備投資が
  稼働率（y/k）で増える
- **公共投資の乗数は小さい**（0.5〜0.7）。同じ輸入関数の投資の項に乗るので、
  長期弾力性 2.9 で半分近くが輸入に漏れる。政府消費との非対称は WP の輸入関数の
  仕様から来ていて、日本向けには輸入関数に政府需要も入れるほうが素直
- **金利を上げると GDP がわずかに増える**。日本の家計は純債権者なので利子所得が
  増え、消費が増える。WP の投資関数は稼働率だけなので設備投資は減らない。
  WP がデンマークで報告する「金利上昇 → 家計の利払い増 → 消費減」とは逆で、
  家計の粗債務の大きさの違いがそのまま出ている
- ショックの初年度にはほとんど反応しない（消費は前期の純資産、投資は前期の
  稼働率で決まる）
...
python models/denmark/final_test.py    # ファイナルテスト → dk_final.csv / dk_final.png
python models/denmark/shocks.py        # 財政・金利ショック → dk_shocks.md / dk_shocks.png
...
- ライセンスはリポジトリ root の `LICENSE`（MIT License）
